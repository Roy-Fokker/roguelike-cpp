...
*** libTCOD Test file/executable
  ...
  [[https://github.com/Roy-Fokker/roguelike/tree/f9022f035ffb9e36ed6bdbbfad7e1303180249e4][Git Repo]]
* Drawing Player and Moving around
  *NOTE*: A word about the code and explainations, I won't be explaining every feature of C++ being used. Perhaps, you are new to C++, but not new to programming, code should make sense to you. For any keywords, or classes from Standard Library that are new for you, I suggest looking it up in [[https://www.cppreference.com][Cpp Reference]]. In general, I write C++ heavily influenced by [[http://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines][C++ Core Guidelines]], code is formatted in [[https://en.wikipedia.org/wiki/Indentation_style#Allman_style][Allman style]]. You might prefer some other styles/means, feel free to write in a way that is comfortable to you.
  ...
  |--------+---------------------------------------------------------------------------------------------------------------------------------------|
  |  21-46 | This function handles our input. I am using "Trailing Return Type" format for function signature.                                     |
  ...
  |--------+---------------------------------------------------------------------------------------------------------------------------------------|
  |     24 | ~libtcod~ is actually a C library. So it tends to define enums in a C-style. Generally, in C++ it's recommended to use strong enum.   |
  ...
  |--------+---------------------------------------------------------------------------------------------------------------------------------------|
  |     57 | use the variables we defined above to launch the console.                                                                             |
  |--------+---------------------------------------------------------------------------------------------------------------------------------------|
  |  61-63 | Some game specific variables,                                                                                                         |
  |        | If ~exit_game~ is true, we will exit the loop                                                                                         |
  |        | ~player_x~ and ~player_y~ will control where the "@" character will be drawn.                                                         |
  |--------+---------------------------------------------------------------------------------------------------------------------------------------|
  |     64 | C++ defines =not=, =or=, =and=, et.al as keywords. I prefer to use these in =if= statements, imo makes the condition easier to read.  |
  ...
  |--------+---------------------------------------------------------------------------------------------------------------------------------------|
  |     66 | Call our input handling function, that we defined on line 21.                                                                         |
  |--------+---------------------------------------------------------------------------------------------------------------------------------------|
  |  67-87 | Simple =switch= on =action=. ~.first~ here reads the 1st value of the pair that we returned from =handle_input=.                      |
  |--------+---------------------------------------------------------------------------------------------------------------------------------------|
  |     76 | because 2nd value of our pair is a =std::any= we have to use ~std::any_cast~ to go back to the type we put in.                        |
  |--------+---------------------------------------------------------------------------------------------------------------------------------------|
  |  77-78 | Update the value of player position using first and second value of the pair we got from line 68.                                     |
  |--------+---------------------------------------------------------------------------------------------------------------------------------------|
  |  81-85 | Toggle fullscreen mode, by simple negation of current window state.                                                                   |
  |--------+---------------------------------------------------------------------------------------------------------------------------------------|
  |     91 | Update the position of "@" using ~player_x~ and ~player_y~.                                                                           |
  |--------+---------------------------------------------------------------------------------------------------------------------------------------|
  |     95 | Lastly, we have to call ~TCOD_quit()~ to properly exit the game.                                                                      |
  |--------+---------------------------------------------------------------------------------------------------------------------------------------|

  [[https://github.com/Roy-Fokker/roguelike/tree/44243e03c1b23bf43eec4efc21542980456219bd][Git Repo]]
* Entity System
