...
** Action handling
	...
	case, where =.second= is of =position= type and is used to update
	player position. 

	=position= type has two members =x= and =y=, representing the offset
	value to move the player in for respective axis.
...
** Key press handling
   ...
   2. one data object held by =std::any= type. Depending on the action, 
	   data can be either a =position= type, or =nullptr=
